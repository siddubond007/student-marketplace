...
const workerDir = path.join('backend', 'src', 'workers');
if (!fs.existsSync(workerDir)) {
  fs.mkdirSync(workerDir, { recursive: true });
}

// 2. Write the Background Worker Script
const workerCode = `const { PrismaClient } = require('@prisma/client');
...
processMatureOrders();
`;
fs.writeFileSync(path.join(workerDir, 'escrowReleaseWorker.js'), workerCode);

// 3. Patch package.json to include a worker script
const packageJsonPath = 'backend/package.json';
let packageJson = fs.readFileSync(packageJsonPath, 'utf8');
if (!packageJson.includes('"worker":')) {
  packageJson = packageJson.replace(
    '"start": "node src/server.js"',
    '"start": "node src/server.js",\n    "worker": "node src/workers/escrowReleaseWorker.js"'
  );
  fs.writeFileSync(packageJsonPath, packageJson);
}

console.log("✅ Background Worker deployed and integrated into package.json!");
