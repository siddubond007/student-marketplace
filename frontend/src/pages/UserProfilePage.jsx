...
export default function UserProfilePage({ currentUser }) {
  ...
  const isVerified = profileUser?.isCollegeVerified || 
                     ...
                     profileUser?.verifications?.collegeStudentId === 'verified' ||
                     profileUser?.verifications?.collegeId === 'verified' ||
                     profileUser?.collegeVerificationStatus === 'verified' ||
                     profileUser?.collegeIdStatus === 'verified';
                     
  const isUnderReview = profileUser?.collegeIdStatus === 'under_review' || 
                        profileUser?.collegeIdStatus === 'pending' || 
                        profileUser?.verifications?.collegeStudentId === 'under_review' || 
                        profileUser?.verifications?.collegeId === 'under_review' || 
                        profileUser?.collegeVerificationStatus === 'under_review' ||
                        profileUser?.collegeIdUnderReview ||
                        Boolean(profileUser?.collegeIdImage) ||
                        Boolean(profileUser?.collegeIdDoc) ||
                        Boolean(profileUser?.collegeIdFile);

  if (isVerified) {
    return (
      <div className="flex items-center gap-1.5 text-emerald-400 text-xs font-medium mt-1">
        <span>✓</span> Student Status Verified
      </div>
    );
  } else if (isUnderReview) {
    return (
      <div className="flex items-center gap-1.5 text-amber-400 text-xs font-medium mt-1">
        <span>⏳</span> Student ID Under Review
      </div>
    );
  } else {
    return (
      <div className="flex items-center gap-1.5 text-slate-400 text-xs font-medium mt-1">
        <span>⚠️</span> Student Status Not Verified
      </div>
    );
  }
})()}
